...
st.set_page_config(page_title="XRP Bloomberg FORCE RÉELLE", layout="wide")
st.markdown("""
    <style>
...
if 'bots' not in st.session_state:
    ...
    if mem:
        ...
        st.session_state.bots = {f"B{i+1}": {"status": "LIBRE", "pa": 0.0, "pv": 0.0, "budget": 30.0, "gain": 0.0, "oid": "NONE"} for i in range(100)}
...
with st.sidebar:
    ...
    b_val = st.number_input("BASE USDC (MIN 30)", value=30.0)
    
    if st.button("🚨 RESET TOTAL (NETTOYAGE)"):
        st.session_state.bots = {f"B{i+1}": {"status": "LIBRE", "pa": 0.0, "pv": 0.0, "budget": 30.0, "gain": 0.0, "oid": "NONE"} for i in range(100)}
        st.session_state.profit_total = 0.0
    ...
    for i in range(100):
        id_bot = f"B{i+1}"
        c1, c2 = st.columns(2)
        if st.session_state.bots[id_bot]["status"] == "LIBRE":
            if c1.button(f"GO {i+1}", key=f"g{i}"):
                ...
                st.session_state.bots[id_bot].update({"status": "ACHAT", "pa": pa_f, "pv": pv_f, "budget": b_val, "oid": "WAITING"})
                sauvegarder(st.session_state.bots, st.session_state.profit_total); st.rerun()
        else:
            if c2.button(f"OFF {i+1}", key=f"o{i}"):
                st.session_state.bots[id_bot].update({"status": "LIBRE", "oid": "NONE"}); st.rerun()
...
while True:
    try:
        ...
        with live.container():
            st.write(f"### MARKET: {px:.4f} | BANKROLL: {st.session_state.bankroll:.2f} USDC")
            c1, c2 = st.columns(2)
            c1.metric("NET GAIN", f"+{st.session_state.profit_total:.4f}")
            st.divider()
            ...
            for name in actifs:
                ...
                # --- SÉCURITÉ VOLUME MINIMUM ---
                if vol < 10.5:
                    st.error(f"❌ {name}: {vol:.2f} XRP trop bas (Min 10.5). RESET & augmente le Budget !")
                    continue
                ...
                if bot["status"] == "ACHAT" and px <= bot["pa"]:
                    try:
                        ...
                        st.success(f"✅ ORDRE PLACÉ : {name}")
                    except Exception as e:
                        if "Post-only" in str(e): st.warning(f"⏳ {name}: Prix trop proche...")
                ...
                # --- VENTE ---
                    ...
                    try:
                        ...
                        sauvegarder(st.session_state.bots, st.session_state.profit_total)
                        st.success(f"💰 VENTE RÉUSSIE : {name}")
                    except Exception as e:
                        st.error(f"❌ KRAKEN {name}: {str(e)[:50]}")

                sc = "#FFA500" if bot["status"] == "ACHAT" else "#00FF00"
                st.markdown(f'<div class="bot-line"><span>{name} [{bot["oid"]}]</span><span style="color:{sc};">{bot["status"]}</span><span>{bot["pa"]}->{bot["pv"]}</span><span class="flash-box">{actuel_b:.2f}$</span></div>', unsafe_allow_html=True)
                time.sleep(0.1)
