...
# 1. MÉMOIRE DES BOTS ET DU PROFIT
if 'active_bots' not in st.session_state: st.session_state.active_bots = {1:True, 2:True, 3:True} 
...
try:
    ...
    # 3. INTERFACE DES 3 BOTS (DÉTECTION PAR VOLUME)
    ...
    for i in range(3):
        ...
        # DÉTECTION INFAILLIBLE : On cherche si un ordre (Achat ou Vente) de ~11.7 XRP existe
        # On définit le volume cible pour ce bot (ex: 11.7)
        vol_cible = 11.7 
        
        mission_active = False
        montant_reel = 0.0
        for o in orders:
            vol_ordre = float(o['amount'])
            # On accepte une petite marge (ex: entre 11.2 et 12.2)
            if abs(vol_ordre - vol_cible) < 0.5:
                mission_active = True
                montant_reel = vol_ordre * float(o['price'])
                break
        ...
        with st.expander(f"🚜 BOT {p_idx} | {status_txt}{label_cash}", expanded=(i==0)):
            ...
            if col_s.button(f"🗑️ STOP B{p_idx}", key=f"stop{i}"):
                ...
                for o in orders:
                    vol_o = float(o['amount'])
                    if abs(vol_o - vol_cible) < 0.5:
