...
# 1. INITIALISATION DES VERROUS (Pour empêcher la relance forcée)
if 'bot_active' not in st.session_state:
    st.session_state.bot_active = {1: False, 2: False, 3: False}

st.set_page_config(page_title="XRP Sniper Safe", layout="centered")
st.markdown("""
    <style>
...
try:
    ...
    # HEADER
    st.markdown(f'<div class="status-box"><h2>SOLDE LIBRE : {usdc_dispo:.2f} $</h2></div>', unsafe_allow_html=True)
    ...
    for i in range(3):
        ...
        # Vérifier si un ordre existe déjà sur Kraken pour ce prix
        a_un_ordre = any(float(o['price']) == p_cible for o in orders)
        
        status = "🟢 ACTIF" if a_un_ordre else "🔴 ARRÊTÉ"
        ...
        with st.expander(f"🚜 BOT {p_idx} | {status}", expanded=(i==0)):
            ...
            # --- LA SÉCURITÉ : AUTO-RELANCE SEULEMENT SI LE BOT EST ACTIVÉ ---
            ...
            # BOUTON LANCER : On active le verrou
            if col_l.button(f"🚀 LANCER B{p_idx}", key=f"run{i}"):
                st.session_state.bot_active[p_idx] = True # ON ALLUME LE VERROU
                st.rerun()

            # BOUTON STOP : On coupe le verrou (Il ne se relancera plus tout seul)
            if col_s.button(f"🗑️ STOP B{p_idx}", key=f"stop{i}"):
                st.session_state.bot_active[p_idx] = False # ON COUPE LE VERROU
                for o in orders:
