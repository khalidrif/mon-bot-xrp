...
# 1. MÉMOIRE DE SÉCURITÉ (ANTI-DOUBLON & PROFITS)
...
try:
    ...
    for i in range(3):
        ...
        status_label = "🟢 BOULE DE NEIGE" if deja_en_cours else "⚪ INACTIF"
        ...
        with st.expander(f"🚜 BOT {p_idx} | {status_label}", expanded=(i==0)):
            ...
            # --- BOUTON LANCER (PROTECTION 5 SECONDES) ---
            if col_l.button(f"🚀 LANCER B{p_idx}", key=f"run{i}"):
                maintenant = time.time()
                if maintenant - st.session_state.last_click < 5:
                    st.warning("Doublon évité ! Patiente un peu...")
                elif deja_en_cours:
                    st.error("Ordre déjà présent sur Kraken !")
