...
try:
    ...
    for i in range(4):
        ...
        # --- PRÉ-DÉTECTION POUR LA BARRE DU HAUT ---
        mission_active = False
        montant_reel = 0.0
        for o in orders:
            p_o = float(o['price'])
            if abs(p_o - p_in_def) < 0.005 or abs(p_o - p_out_def) < 0.005:
                mission_active = True
        ...
        # TITRE DE LA BARRE : STATUT | MONTANT | CYCLES | GAIN
        titre = f"{status} 📦 {montant_reel:.2f}$ | 🔄 {cyc} | 💰 +{g_gras} | B{p_idx}"

        with st.expander(titre, expanded=(p_idx==1)):
            # --- SAISIE MANUELLE ---
            m_invest = st.number_input(f"MONTANT $ B{p_idx}", value=14.50, step=0.5, key=f"m{p_idx}")
            ...
            if c2.button(f"🗑️ STOP B{p_idx}", key=f"stop{p_idx}"):
                ...
                for o in orders:
                    if abs(float(o['price']) - p_in) < 0.005 or abs(float(o['price']) - p_out) < 0.005:
