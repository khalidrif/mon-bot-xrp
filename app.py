...
# === VERROU GLOBAL import streamlit as st
import ccxt
import json
import os
import time
...
# === LOGS ===
# Ligne 17 et 18 : On gère les logs
if "logs" not in st.session_state:
    st.session_state.logs = []  # <--- Cette ligne doit être décalée !

# Ligne 19 et 20 : On gère ton nouveau verrou
if "achat_en_cours" not in st.session_state:
    st.session_state.achat_en_cours = False  # <--- Celle-ci aussi !
    ...
    if b["etape"] == "ACHAT" and mid <= b["p_achat"]:
        if not verrou_global["achat_en_cours"] and usdc >= (b["mise"] + 0.50):
            ...
            try:
                ...
                log(f"✅ Bot {i} : Achat UNIQUE envoyé")
                b["etape"] = "VENTE"
                save_bots()
            except Exception as e:
