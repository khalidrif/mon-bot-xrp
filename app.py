...
# === CONFIGURATION ===
st.set_page_config(page_title="⚡ XRP Sniper Live (Finale Stable)", layout="centered")
symbol = "XRP/USDC"
...
def load_bots():
    ...
    try:
        ...
        log(f"⚠️ Fichier JSON corrompu renommé en {corrupt_name}")
        return {}
    except Exception as e:
...
try:
    ...
    bid = ask = mid = 0.0
    log(f"⚠️ Kraken erreur ticker : {e}")
...
# === EN-TÊTE ===
st.title("🚀 XRP Sniper Live Trading (version finale)")
...
with col1:
    p_achat_new = st.number_input("Prix Achat", value=1.390, step=0.0001)
with col2:
    p_vente_new = st.number_input("Prix Vente", value=1.400, step=0.0001)
with col3:
...
for i, b in st.session_state.bots.items():
    ...
    # --- Sécurisation du paramètre de précision ---
    try:
