...
def bot_thread(prix_achat, prix_vente, montant_usdc_initial, log, profit_box, history_box, snowball):
    ...
    while running:
        ...
        texte += f"Trade : {montant_usdc:.4f} USDC → {montant_xrp:.4f} XRP\n"
        texte += f"Profit net : {profit_net:.4f} USDC\n"

        solde_usdc = get_usdc_balance()
        if position == 0 and solde_usdc < montant_usdc:
            st.warning(f"Solde insuffisant ({solde_usdc} USDC)")
            time.sleep(3)
            continue
        ...
        if position == 0 and prix <= prix_achat:
            ...
            if snowball:
                montant_usdc += gain
                texte += f"\nBOULE DE NEIGE : {montant_usdc:.4f} USDC\n"

        log.text(texte)

        try:
            df_hist = get_order_history()
            history_box.dataframe(df_hist)
        except:
            pass
...
snowball = st.checkbox("Activer Boule de Neige (réinvestissement automatique)")
