...
import json
import os
from config import get_kraken_connection # Import de ton fichier config
...
# On appelle la fonction de config.py
        ...
        if st.session_state.bots[name]["status"] == "LIBRE":
            if c1.button(f"GO {i+1}", key=f"on_{i}"):
                try:
                    ...
                    st.rerun()
                except Exception as e: st.error(f"Erreur: {str(e)[:40]}")
        else:
...
while True:
    try:
                    ...
                    if bot["id"]:
                        ...
                        if order['status'] == 'closed':
                            if bot["status"] == "ACHAT":
                                ...
                                nq = float(kraken.amount_to_precision('XRP/USDC', nm / bot['p_achat']))
                                res = kraken.create_order('XRP/USDC', 'limit', 'buy', nq, bot['p_achat'], {'validate': not mode_reel})
        ...
        st.caption(f"API Flux : {str(e)[:30]}...")
